...
public interface StockRepositroy extends JpaRepository<StockEntity, Long> {
    ...
    @Query(value="SELECT a.*, mean_match_status, mean_price_error, tod_return\n" +
            ...
            "FROM stock_predict WHERE DATE = '2020-05-29') AS a LEFT OUTER JOIN(SELECT com_name,  round(AVG(MATCH_status),2) AS mean_match_status, round(AVG(price_error/tod_price)*100) AS mean_price_error , round(EXP(SUM(LOG(stock_predict.return))),2) AS tod_return\n" +
            "FROM stock_predict WHERE DATE > '2020-05-30'- INTERVAL 90 DAY GROUP BY com_name) AS b  ON a.com_name = b.com_name" ,nativeQuery = true)
